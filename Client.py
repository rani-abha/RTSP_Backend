...
class Client:
	...
	def __init__(self, master, serveraddr, serverport, rtpport, filename):
		...
		self.createVisualizationFrames()
	...
	def createVisualizationFrames(self):
		"""Create frames to visualize TCP/IP layers."""
		self.application_layer_frame = Frame(self.master, bd=2, relief=GROOVE)
		self.application_layer_frame.grid(row=2, column=0, columnspan=4, padx=2, pady=2, sticky=W+E)
		...
		self.network_layer_frame.grid(row=4, column=0, columnspan=4, padx=2, pady=2, sticky=W+E)
		Label(self.network_layer_frame, text="Network Layer (IP)").pack()
  
		self.application_packet_label = Label(self.application_layer_frame, text="", anchor=W, justify=LEFT)
		self.application_packet_label.pack(fill=BOTH)
	...
	def updateVisualization(self,layer, data):
		"""Update visualization for the given layer."""
	...
	def exitClient(self):
		...
		if CACHE_FILE_NAME:os.remove(CACHE_FILE_NAME + str(self.sessionId) + CACHE_FILE_EXT) # Delete the cache image from video
	...
	def sendRtspRequest(self, requestCode):
		...
		if requestCode == self.SETUP and self.state == self.INIT:
			...
			self.updateVisualization("application", request)  # Update visualization
	...
	def recvRtspReply(self):
		...
		while True:
			...
			if reply: 
				...
				self.updateVisualization("transport", reply.decode("utf-8"))  # Update visualization
